...
public class DeviceProvider extends ContentProvider{
	...
	private String[] mTables = new String[]{
			...
			DeviceDBHelper.TABLE_NAME_DEVICE_PINPAI_TUIJIAN,
			DeviceDBHelper.TABLE_NAME_DEVICE_NPINPAI,
			DeviceDBHelper.TABLE_NAME_DEVICE_VOICE_PINPAI,
			DeviceDBHelper.TABLE_NAME_policy,
	};
	...
	private static final int HAIER_REGION = 0x0300;
	private static final int HAIER_REGION_CODE = 0x0301;
	
	/**关闭设备数据库*/
	private static final int CLOSE_DEVICE = 0x0312;
	
	private static final int PINPAI_TUIJIAN = 0x0400;
	private static final int PINPAI_TUIJIAN_ID = 0x0401;
	
	private static final int PINPAI_NPINPAI = 0x0501;

	private static final int PINPAI_VOICE = 0x0601;


	private static final int POLICY = 0x0700;
	private static final int POLICY_UPCODE = 0x0701;
	...
	private static final UriMatcher sURIMatcher = new UriMatcher(UriMatcher.NO_MATCH);
	 static {
	        ...
	        matcher.addURI(BjnoteContent.DEVICE_AUTHORITY, "pinpai/tuijian", PINPAI_TUIJIAN);
	        matcher.addURI(BjnoteContent.DEVICE_AUTHORITY, "pinpai/tuijian/#", PINPAI_TUIJIAN_ID);
	        
	        matcher.addURI(BjnoteContent.DEVICE_AUTHORITY, "pinpai/npinpai", PINPAI_NPINPAI);

		 matcher.addURI(BjnoteContent.DEVICE_AUTHORITY, "pinpai/voice", PINPAI_VOICE);


		 matcher.addURI(BjnoteContent.DEVICE_AUTHORITY, "policy", POLICY);
		 matcher.addURI(BjnoteContent.DEVICE_AUTHORITY, "policy/#", POLICY_UPCODE);
    ...
    private void notifyChange(int match) {
		...
		case HAIER_REGION_CODE:
			...
			break;
		case PINPAI_TUIJIAN:
		case PINPAI_TUIJIAN_ID:
			notify = BjnoteContent.PinPai.BX_TUIJIAN_CONTENT_URI;
			break;
		case PINPAI_NPINPAI:
			notify = BjnoteContent.PinPai.BX_NPINPAI_CONTENT_URI;
			break;
			case PINPAI_VOICE:
				notify = BjnoteContent.PinPai.VOICE_PINPAI_CONTENT_URI;
				break;
			case POLICY:
			case POLICY_UPCODE:
				notify = BjnoteContent.Policy.CONTENT_URI;
				break;
    	}
...
//			case PINPAI_TUIJIAN_ID:
	...
	public Uri insert(Uri uri, ContentValues values) {
		...
		SQLiteDatabase db = getDatabase(context);
		long id = -1;
		if (values.containsKey(DeviceDBHelper.ID)) {
			values.remove(DeviceDBHelper.ID);
		}
		 switch(match) {
			 case POLICY:
			 case POLICY_UPCODE:
...
//         Context context = getContext();
//         // See the comment at delete(), above
//         SQLiteDatabase db = getAppDatabase(context);
         ...
         switch(match) {
			...
			case PINPAI_TUIJIAN_ID:
			 ...
			 case POLICY_UPCODE:
        	     ...
        	     break;
			case PINPAI_NPINPAI:
				result = db.query(table, projection, selection, selectionArgs, DeviceDBHelper.DEVICE_PINPAI_NAME, null, sortOrder);
				break;
			case CLOSE_DEVICE:
				closeDatabase();
				break;
